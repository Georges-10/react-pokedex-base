...
type PokeC = HTMLAttributes<HTMLDivElement> & {
  pokemon: Pokemon;
  details?: boolean;
...
export default (function Pokecard({
  ...
  if (details) {
    return (
      <div
        ref={ref}
        className={`pokecard text-base relative flex md:flex-row flex-col items-center justify-between p-5 m-5 rounded-lg shadow-2xl mt-10 ${pokemon.types[0]} overflow-hidden`}
      >
        {/* Infos */}
        <div className="flex flex-col gap-1 md:w-6/12 w-full md:order-1 order-2 md:mt-0 mt-5 md:text-left text-center">
          ...
          <div className="types flex gap-1 text-white text-sm md:justify-normal justify-center mt-1">
            {pokemon.types.map((type) => (
              <div
                ...
                className={`rounded px-1.5 py-1 ${type} flex items-center gap-1`}
              >
                <img
                  src={`/types/${type}.svg`}
                  alt={type}
          ...
          <div className="bg-black bg-opacity-40 p-5 rounded-lg mt-5 text-left">
            ...
            <div className="stats flex flex-col gap-1 text-white text-base mt-4">
              {Object.entries(pokemon.states).map(([key, value]) => (
                <div
                  key={`${pokemon.name}-${key}`}
                  className={`rounded px-1.5 py-1 ${key} flex items-center justify-between gap-10`}
                >
  ...
  return (
    <NavLink to={`/pokemon/${pokemon.id}`}>
      <div
        ref={ref}
        className={`pokecard relative flex items-center hover:scale-105 duration-150 justify-between p-5 cursor-pointer rounded-lg shadow-2xl h-[170px] ${pokemon.types[0]}`}
      >
        {/* Infos */}
